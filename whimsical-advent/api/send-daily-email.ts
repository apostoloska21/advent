...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (error) {
      ...
      // Log the failed email attempt (console only since no database)
      console.log(`❌ Email failed for day ${currentDay} to ${recipientEmail}: ${error.message}`);
      return res.status(500).json({ error: `Failed to send email: ${error.message}` });
    }

    // Log successful email send (console only since no database)
    console.log(`✅ Email sent successfully for day ${currentDay} to ${recipientEmail}`);
