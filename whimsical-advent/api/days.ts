...
import { adventDays } from '../src/lib/schema';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    // Seed database if needed (only happens once)
    ...
    if (dayNumber) {
      // Return specific day
      const dayResult = await db.select().from(adventDays).where(eq(adventDays.day, dayNumber));
      if (dayResult.length === 0) {
        return res.status(404).json({ error: 'Day not found' });
      }
      return res.status(200).json({ day: dayResult[0] });
    } else {
      // Return all days
      const allDays = await db.select().from(adventDays).orderBy(asc(adventDays.day));
      return res.status(200).json({ days: allDays });
    }
  } catch (error) {
